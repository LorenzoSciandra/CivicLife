...
public class AuthController {
        ...
        if(!tokens.isEmpty()){
            for (Optional<Token> optionalToken : tokens) {
                if(optionalToken.isPresent()){
                    ...
                    String token = getMostRecentToken(tokenBean);
                    result.append(tokenBean.toString(token));
                }
            }
        }
    ...
    public String getToken(@PathVariable(value = "email") String email,
        ...
        if (optionalTokenBean.isPresent()){
            ...
            removeExpired(tokenBean);
            tokenRepository.save(tokenBean);
            result = tokenBean.toString(getMostRecentToken(tokenBean));
        }
        return result;
    }
    ...
    private void removeExpired(Token tokenBean){
        Instant now = Instant.now();
        HashMap<String, Instant> tokens = tokenBean.getTokens();
    ...
    private String getMostRecentToken(Token tokenBean){
